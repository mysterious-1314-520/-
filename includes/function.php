...
function get_ip_city($ip)
{
    ...
    if (!empty($city['city'])) {
        ...
        $location = isset($city['province']) ? $city['province'] : '';
    }
	if($location){
		return $location;
	}else{
...
function daddslashes($string, $force = 0, $strip = FALSE) {
	...
	if(!MAGIC_QUOTES_GPC || $force) {
		if(is_array($string)) {
			...
			$string = addslashes($strip ? stripslashes((string)$string) : (string)$string);
		}
	}
	return $string;
}
...
function checkmobile() {
	...
	if((dstrpos($useragent, $ualist) || strexists($accept, "VND.WAP") || strexists($via,"wap"))){
		return true;
	}else{
		return false;
	}
...
function random($length, $numeric = 0) {
	$document_root = isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : ROOT;
...
function saveSetting($k, $v){
	global $DB;
	$v = daddslashes($v);
	return $DB->query("REPLACE INTO web_config SET v='$v',k='$k'");
}
...
function checkIfActive($string) {
	...
	$dot_pos = strrpos($request_uri,'.');
	$php_self = ($slash_pos !== false && $dot_pos !== false && $dot_pos > $slash_pos) ? substr($request_uri,$slash_pos+1,$dot_pos-$slash_pos-1) : '';
	if (in_array($php_self,$array)){
		return 'active';
	}else
		return null;
}
function writeLog($action, $target, $target_id=0, $detail=''){
	global $DB;
	$ip = real_ip();
	$DB->query("INSERT INTO web_log (action,target,target_id,detail,ip,addtime) VALUES ('$action','$target','$target_id','$detail','$ip','".time()."')");
}
?>
