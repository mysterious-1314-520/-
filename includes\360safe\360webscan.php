...
webscan_error();
//引用配置文件
require_once __DIR__ . '/webscan_cache.php';
//防护脚本版本号
...
class webscan_http {
  ...
  function request($url, $referer = '', $limit = 0, $timeout = 30, $block = TRUE) {
    ...
    $out .= "User-Agent: ".(isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '')."\r\n";
    $out .= "Host: $host\r\n";
    if($this->method == 'POST') {
...
function webscan_cheack() {
  if(isset($_POST['webscan_rkey']) && $_POST['webscan_rkey']==WEBSCAN_U_KEY){
    return true;
  }
  return false;
}
...
function webscan_StopAttack($StrFiltKey,$StrFiltValue,$ArrFiltReq,$method) {
  ...
  if (preg_match("/".$ArrFiltReq."/is",$StrFiltValue)==1){
    webscan_slog(array('ip' => isset($_SERVER["REMOTE_ADDR"]) ? $_SERVER["REMOTE_ADDR"] : '','time'=>date("Y-m-d H:i:s"),'page'=>isset($_SERVER["PHP_SELF"]) ? $_SERVER["PHP_SELF"] : '','method'=>$method,'rkey'=>$StrFiltKey,'rdata'=>$StrFiltValue,'user_agent'=>isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '','request_url'=>isset($_SERVER["REQUEST_URI"]) ? $_SERVER["REQUEST_URI"] : ''));
    exit(webscan_pape());
  }
  if (preg_match("/".$ArrFiltReq."/is",$StrFiltKey)==1){
    webscan_slog(array('ip' => isset($_SERVER["REMOTE_ADDR"]) ? $_SERVER["REMOTE_ADDR"] : '','time'=>date("Y-m-d H:i:s"),'page'=>isset($_SERVER["PHP_SELF"]) ? $_SERVER["PHP_SELF"] : '','method'=>$method,'rkey'=>$StrFiltKey,'rdata'=>$StrFiltKey,'user_agent'=>isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '','request_url'=>isset($_SERVER["REQUEST_URI"]) ? $_SERVER["REQUEST_URI"] : ''));
    exit(webscan_pape());
  }
